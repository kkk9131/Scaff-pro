...
- 表示タブ（上部）  
  ...
  - 「アイソメ」  
    - 平面図＋立面情報から自動生成された建物アイソメ図を表示。  
    - リアルな建物表現（窓・模様など）は行わず、輪郭線と屋根形状・玄関など開口位置がわかる程度のシンプルな線画表現。  
    - 足場を重ねて表示可能。  
  - 「パース」  
    - 建物＋足場を透視図法で表示。  
    - ワイヤーフレーム表現を維持しつつ、遠近感のある表示を提供。

### 3.3.1 参照用図面ビューア（フローティングパネル）

- インポートした建物図面（平面図・立面図）を、メインキャンバスとは別のフローティングウィンドウとして表示。  
- 機能  
  - 表示/非表示の切り替え（閉じる）  
  - ウィンドウのリサイズ（縮小・拡大）  
  - ドラッグによる位置移動  
- 目的  
  - メインキャンバスでの足場作図作業中に、元の建物図面を参照しながら作業できるようにする。  
  - 図面を見ながら正確な外周設定や開口位置の確認が可能。
